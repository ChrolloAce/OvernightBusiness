...
export class CentralizedDataLoader {
  ...
  static async loadAnalytics(profile: SavedBusinessProfile, options: {
    ...
    try {
      ...
      // Handle different formats: accounts/123/locations/456 or just the location ID
      ...
      if (locationMatch) {
        ...
        // If it's a full resource name but doesn't match expected pattern
        ...
        if (locationIndex >= 0 && locationIndex + 1 < parts.length) {
          ...
          console.error('[CentralizedDataLoader] Cannot extract location ID from:', profile.googleBusinessId)
          return { success: false, error: `Invalid business ID format: ${profile.googleBusinessId}` }
        }
      }
      
      console.log('[CentralizedDataLoader] Using location ID for analytics:', locationId)
      ...
      console.log('[CentralizedDataLoader] Fetching analytics with params:', {
        locationId,
      ...
      console.log('[CentralizedDataLoader] Analytics API response:', data)
