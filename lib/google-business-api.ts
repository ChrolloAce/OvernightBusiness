...
// Q&A related interfaces
export interface BusinessQuestion {
  name: string
  author: {
    displayName: string
    profilePhotoUri?: string
    type: 'MERCHANT' | 'REGULAR_USER' | 'LOCAL_GUIDE'
  }
  upvoteCount: number
  text: string
  createTime: string
...
export class GoogleBusinessAPI {
  ...
  // Q&A Methods
  async getQuestions(accountId: string, locationId: string, pageSize: number = 50, pageToken?: string): Promise<QuestionsResponse> {
    console.log(`[GoogleBusinessAPI] Loading questions for location: ${locationId}`)
    ...
    try {
      ...
      const response = await fetch(
        `https://mybusinessqanda.googleapis.com/v1/locations/${locationId}/questions?${params}`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        ...
        console.error(`[GoogleBusinessAPI] Questions API error:`, response.status, errorData)
        throw new Error(`Failed to fetch questions: ${response.status}`)
      }

      const data = await response.json()
      console.log(`[GoogleBusinessAPI] Loaded ${data.questions?.length || 0} questions`)
      ...
      console.error('[GoogleBusinessAPI] Error loading questions:', error)
      throw error
    }
  }

  async getQuestionAnswers(questionName: string, pageSize: number = 50, pageToken?: string): Promise<{ answers: BusinessAnswer[], nextPageToken?: string }> {
    console.log(`[GoogleBusinessAPI] Loading answers for question: ${questionName}`)
    ...
    try {
      ...
      const response = await fetch(
        `https://mybusinessqanda.googleapis.com/v1/${questionName}/answers?${params}`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorData = await response.text()
        console.error(`[GoogleBusinessAPI] Answers API error:`, response.status, errorData)
        throw new Error(`Failed to fetch answers: ${response.status}`)
      }

      const data = await response.json()
      console.log(`[GoogleBusinessAPI] Loaded ${data.answers?.length || 0} answers`)
      ...
      console.error('[GoogleBusinessAPI] Error loading answers:', error)
      throw error
    }
  }

  async createQuestion(locationId: string, questionText: string): Promise<BusinessQuestion> {
    console.log(`[GoogleBusinessAPI] Creating question for location: ${locationId}`)
    ...
    try {
      ...
      const response = await fetch(
        `https://mybusinessqanda.googleapis.com/v1/locations/${locationId}/questions`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
      ...
      if (!response.ok) {
        const errorData = await response.text()
        console.error(`[GoogleBusinessAPI] Create question error:`, response.status, errorData)
        throw new Error(`Failed to create question: ${response.status}`)
      }

      const data = await response.json()
      console.log(`[GoogleBusinessAPI] Created question successfully`)
      
      return data
    } catch (error) {
      console.error('[GoogleBusinessAPI] Error creating question:', error)
      throw error
    }
  }

  async answerQuestion(questionName: string, answerText: string): Promise<BusinessAnswer> {
    console.log(`[GoogleBusinessAPI] Answering question: ${questionName}`)
    ...
    try {
      ...
      const response = await fetch(
        `https://mybusinessqanda.googleapis.com/v1/${questionName}/answers:upsert`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
      ...
      if (!response.ok) {
        const errorData = await response.text()
        console.error(`[GoogleBusinessAPI] Answer question error:`, response.status, errorData)
        throw new Error(`Failed to answer question: ${response.status}`)
      }

      const data = await response.json()
      console.log(`[GoogleBusinessAPI] Answered question successfully`)
      
      return data
    } catch (error) {
      console.error('[GoogleBusinessAPI] Error answering question:', error)
      throw error
    }
  }
} 