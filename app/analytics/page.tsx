...
export default function AnalyticsPage() {
  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        console.log('[Analytics] Raw analytics data:', result.data)
  ...
  const processAnalyticsData = (rawData: any): AnalyticsData => {
    ...
    dailyData.forEach((series: any) => {
      ...
      if (series.dailyBreakdowns) {
        series.dailyBreakdowns.forEach((breakdown: any) => {
          const date = breakdown.date
          ...
          const value = parseInt(breakdown.value || '0')
